...
// quickest sprite animation is 11 frames a second. Linux's native window path is
// also capped at 30 moves a second. The stationary Windows stage updates Kairo's
// transform separately at display rate without redrawing unchanged canvas art.
...
function catOrigin() {
  if (isPreview || stage.full) return { x: engine.x, y: engine.y };
...
function updateView(now) {
  ...
  // A stationary Windows overlay can follow the display refresh rate without a
  // native window move or IPC round trip. Canvas frames remain capped below;
  // these writes are gated by rounded pixel positions.
  if (stage.full) {
    layOutCat();
    if (engine.speech) updateBubble();
...
function draw(now) {
  ...
  if (isPreview || stage.full) return;
...
function hitsCatAt(clientX, clientY) {
  const origin = catOrigin();
  let column = Math.floor(((clientX - origin.x) / engine.options.catWidth) * SPRITE_WIDTH);
  const row = Math.floor(((clientY - origin.y) / engine.options.catHeight) * SPRITE_HEIGHT);
...
function hitsCat(event) {
  ...
  return hitsCatAt(event.clientX, event.clientY);
}

function setMousePassthrough(ignore) {
  if (!stage.full || ignore === mousePassthrough) return;
  mousePassthrough = ignore;
  window.desktopPet?.setMousePassthrough(ignore);
}

function syncMousePassthrough() {
  if (!stage.full || engine.dragging || !lastMousePoint) return;
  setMousePassthrough(!hitsCatAt(lastMousePoint.x, lastMousePoint.y));
}

// With `forward: true`, Windows continues to deliver mouse movement while the
// overlay ignores clicks. That lets only Kairo's painted pixels become
// interactive; the rest of the work area remains the user's desktop.
...
catElement.addEventListener("pointerdown", (event) => {
  ...
  if (stage.full) {
    const origin = catOrigin();
    point = {
      x: engine.x + (event.clientX - origin.x),
      y: engine.y + (event.clientY - origin.y)
    };
  } else {
    // Keep the small moving-window coordinate loop byte-for-byte equivalent to
    // the Linux path: its window-relative measurements intentionally cancel
    // part of the window's own motion.
    const bounds = catElement.getBoundingClientRect();
    point = {
